 ...
 * @param data
 */
static void pdu_hashData_function(PduDataPacket *dev,pdu_dev_data *data)
{
    ...
    case PDU_CMD_ENV: //环境数据
        pdu_hashDevData_save(dev->data, data);
        break;
...
{
    static PduHashIP *hashIP = NULL;
    static PduDevHash *devHash = NULL;
    static PduDataPacket *dev = NULL;
    ...
    if(ret)
    {
        ...
        if(devType > 0)
        {
            hashIP =  pdu_hashData_get(devType); // 根据设备代号获取对应的Hash节点
            devHash = hashIP->get(packet->ip); // 根据不同的设备IP 获取对应的Hash节点
            dev = devHash->get(packet->data->addr); // 根据不同的设备号 获取对应的设备Hash节点
            ...
            if(packet->code->version == NET_DATA_VERSION) //版本号的验证
            {
                ...
                dev->offLine = PDU_OFF_LINE_TIME; //设备在线标识

                if(devHash->isNew) // 新设备得加上保存队列中
                {
